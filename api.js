...
const { gitRepoUrl } = config // 获取仓库地址
const s = gitRepoUrl.split('/')// 获取仓库地址
const DEFAULT_BRANCH = config.branch // 默认分支

export const authorName = s[s.length - 2] // 作者名
export const repoName = s[s.length - 1] // 获取仓库名

function isGitee() { // 是否为码云
  return config.provider === 'Gitee'
}

// 验证Token
export function verifyToken(token: string) { // 验证Token
  const url = isSelfDevelop ? '/api/users/verify' : `/users/${authorName}` // 获取仓库地址
  return http.get(url, {
    headers: {
...
export function getContentes() {
  return http.post('/api/contents/get').then((res: any) => {
    // 清空内容
    websiteList.splice(0, websiteList.length) // 清空内容
